...
export default function ReviewPage() {
  ...
  useEffect(() => {
    ...
    setAdditionalInfo(formData.additionalInfo); // Set additionalInfo in context
  ...
  return (
    <div>
          <ProgressBar step={5} />
      <div className="container-fluid px-3 sm:px-4 md:px-5">
        {/* Header Section */}
        ...
        {/* Main Content */}
        <div className="max-w-3xl mx-auto my-5 sm:my-8">
          {/* Selected Items Review */}
          {selections.map((item, index) => (
            <div key={index} className="w-full max-w-lg mx-auto my-3 sm:my-4">
              <div className="flex items-center bg-[#F5F5F5] min-h-[50px] mx-1 py-2 sm:py-3 px-2 sm:px-3 rounded">
                ...
                <div className="flex-1 overflow-hidden pr-1">
                  {Array.isArray(item.label) ? (
                    item.label.map((text, i) => (
                      <div key={i} className="font-['Apercu-Medium'] text-[14px] sm:text-[18px] leading-[18px] sm:leading-[21px] text-black font-bold truncate">
                        {text}
                      </div>
                    ))
                  ) : (
                    <span className="font-['Apercu-Medium'] text-[14px] sm:text-[18px] leading-[18px] sm:leading-[21px] text-black font-bold">
                      {item.label}
                    </span>
                  )}
                </div>
                <div className="w-12 sm:w-16 text-center text-[14px] sm:text-base">
          ...
          <div className="max-w-lg mx-auto mb-8 sm:mb-12 mt-10 sm:mt-16 px-2 sm:px-0 flex flex-col items-center">
          ...
          {/* Your Needs Section */}
          <div className="max-w-lg mx-auto mb-8 sm:mb-12 mt-10 sm:mt-16 px-2 sm:px-0 text-center">
            ...
            <div className="flex flex-wrap gap-3 sm:gap-4 justify-center mb-4 sm:mb-6">
              <div className="w-full sm:w-[calc(50%-0.75rem)] bg-[#1D4045] rounded py-3 sm:py-4 px-3 sm:px-6 border border-white">
                <p className="font-['Apercu'] text-[16px] sm:text-[18px] leading-[19px] sm:leading-[21px] text-white mb-0 flex items-center justify-center">
                  <FaCheck className="w-4 h-4 sm:w-5 sm:h-5 mr-2 sm:mr-3 flex-shrink-0" />
                  <span>Coffee Advice</span>
                </p>
              </div>
              <div className="w-full sm:w-[calc(50%-0.75rem)] bg-[#1D4045] rounded py-3 sm:py-4 px-3 sm:px-6 border border-white">
                <p className="font-['Apercu'] text-[16px] sm:text-[18px] leading-[19px] sm:leading-[21px] text-white mb-0 flex items-center justify-center">
            ...
            <span className="text-[#1D4045] underline cursor-pointer text-[14px] sm:text-base">Edit</span>
          </div>
            ...
            <form className="w-full">
              <div className="text-center">
                <textarea
                  ...
                  onChange={handleInputChange}
                  className="w-full p-3 sm:p-4 border border-[#1D4045] rounded focus:outline-none font-['Apercu'] text-[15px] sm:text-[16px] text-black min-h-[150px] sm:min-h-[200px]"
                  maxLength={750}
                />
              </div>
            </form>
          ...
          <div className="max-w-lg mx-auto my-6 sm:my-8 px-2 sm:px-0">
            <div className="w-full sm:w-4/5 mx-auto text-center p-1 bg-[#1D4045] rounded">
              <label className="flex items-center justify-center text-white cursor-pointer py-2 sm:py-3 px-2 sm:px-0">
                ...
                />
                <span className="uppercase font-bold font-['Apercu-Medium'] text-[13px] sm:text-[16px]">
        ...
        <div className="text-center mb-8 sm:mb-12 px-2 sm:px-0">
          <button
            onClick={handleNext}
            className="w-full sm:w-[300px] h-[45px] sm:h-[50px] bg-yellow-500 text-black uppercase font-['Apercu-Medium'] text-[15px] sm:text-[16px] rounded hover:bg-yellow-600 transition-colors max-w-[300px]"
          >
            Submit
          </button>
        </div>
